...
class ProposalController {
    ...
    /**
     * Generate proposal from proposal request (Financial Manager only)
     */
    public function generateProposalFromRequest() {
        try {
            SessionManager::requireAuth();
            ...
            $requestId = Validator::required($data['request_id'] ?? '', 'Request ID');
            $cropType = Validator::required($data['crop_type'] ?? '', 'Crop type');
            $estimatedYield = Validator::required($data['estimated_yield'] ?? '', 'Estimated yield');
            $leaseDurationYears = Validator::required($data['lease_duration_years'] ?? '', 'Lease duration');
            $rentalValue = Validator::required($data['rental_value'] ?? '', 'Rental value');
            $profitSharingFarmmaster = Validator::required($data['profit_sharing_farmmaster'] ?? '', 'FarmMaster profit sharing');
            ...
            $proposalData = [
                ...
                'generated_from_request_id' => $requestId
            ];

            // Create the proposal
            $proposalId = $this->proposalModel->createProposal($proposalData);
            
            if ($proposalId) {
                // Update the proposal request status
                $updateSql = "UPDATE proposal_requests SET status = 'proposal_generated', 
                              financial_manager_notes = :notes, updated_at = CURRENT_TIMESTAMP 
                ...
                Response::success("Proposal generated successfully from request", [
                    'proposal_id' => $proposalId,
                    'request_id' => $requestId,
                    'message' => 'The landowner will be notified about the new proposal'
                ], 201);
            } else {
                Response::error("Failed to generate proposal");
            }
